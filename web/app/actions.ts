...
export async function recordContributionAction(formData: FormData) {
  ...
  const method =
    (formData.get('method') as 'cash' | 'bank-transfer' | 'eftpos' | 'other') || 'cash';
  const note = formData.get('note') ? String(formData.get('note')) : undefined;
...
export async function updateRoleAction(formData: FormData) {
  ...
  if (formData.has('description')) {
    ...
    payload.description =
      description !== null && String(description).length > 0 ? String(description) : '';
  }
  const permissions = formData.getAll('permissions').map(value => String(value));
...
export async function deleteRoleAction(formData: FormData) {
  const roleId = String(formData.get('roleId'));
  const reassignRoleId = formData.get('reassignRoleId')
    ? String(formData.get('reassignRoleId'))
    : undefined;
  await apiFetch(`/roles/${roleId}`, {
    method: 'DELETE',
    body: JSON.stringify({
...
export async function addChildAction(formData: FormData) {
  ...
  const medicalNotes = formData.get('medicalNotes')
    ? String(formData.get('medicalNotes'))
    : undefined;
