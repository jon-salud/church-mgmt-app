            ...
            <div className="flex space-x-4">
                ...
                <DropdownMenu>
                    <DropdownMenuTrigger className="inline-flex h-10 w-[180px] items-center justify-between whitespace-nowrap rounded-md border border-input bg-background px-3 py-2 text-sm font-medium ring-offset-background transition-colors hover:bg-accent hover:text-accent-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:cursor-not-allowed disabled:opacity-50">
                        {allTypesSelected ? "All Types" : typeFilter.length === 0 ? "No Types Selected" : `${typeFilter.length} selected`}
                    </DropdownMenuTrigger>
                    <DropdownMenuContent className="w-56">
                        ...
                        {requestTypes.map((type) => (
                            <DropdownMenuCheckboxItem
                                ...
                                onCheckedChange={(checked) => {
                                    setTypeFilter(checked ? [...typeFilter, type.id] : typeFilter.filter((id) => id !== type.id));
                                }}
                            >
                                {type.name}
                            </DropdownMenuCheckboxItem>
            ...
            {selectedItem && (
                <Dialog open={!!selectedItem} onOpenChange={() => setSelectedItem(null)}>
                    <DialogContent>
                        ...
                        <div className="p-4">
                            <p>
                                <strong>Type:</strong> {selectedItem.type || typeMap.get(selectedItem.requestTypeId)}
                            </p>
                            <p>
                                <strong>Author:</strong> {selectedItem.author ? `${selectedItem.author.profile.firstName} ${selectedItem.author.profile.lastName}` : "N/A"}
                            </p>
                            <p>
                                <strong>Status:</strong> {selectedItem.status || "Pending"}
                            </p>
                            <p>
                                <strong>Created:</strong> {new Date(selectedItem.createdAt).toISOString().split("T")[0]}
                            </p>
                            <p>
