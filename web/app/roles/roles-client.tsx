        ...
        <section className="space-y-6">
            <header className="flex flex-col gap-3 md:flex-row md:items-end md:justify-between">
                <div className="space-y-1">
                    ...
                    <p className="text-sm text-muted-foreground">Configure role templates and granular permissions. The Admin role is system protected.</p>
                </div>
                <button
                    type="button"
                    ...
                    <tbody className="divide-y divide-border">
                        {sortedRoles.map((role) => (
                            <tr key={role.id} className="transition hover:bg-muted/70">
                                ...
                                <td className="px-4 py-3 text-muted-foreground">{role.permissions.length ? role.permissions.join(", ") : "—"}</td>
            ...
            >
                <form action={createRoleAction} className="grid gap-3" onSubmit={() => setCreateOpen(false)}>
                    <label className="grid gap-1 text-xs uppercase text-muted-foreground">
                        Name
                        <input name="name" required maxLength={100} className="rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground" />
                    </label>
                    <label className="grid gap-1 text-xs uppercase text-muted-foreground">
                        Description
                        <input name="description" maxLength={255} className="rounded-md border border-border bg-background px-3 py-2 text-sm text-foreground" />
                    </label>
                    ...
                    <div className="flex justify-end gap-2">
                        <button type="button" onClick={() => setCreateOpen(false)} className="rounded-md border border-border px-4 py-2 text-sm text-foreground transition hover:bg-muted">
                            Cancel
                        </button>
                        <button type="submit" className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition hover:bg-primary/90">
                            Save Role
                        </button>
                    </div>
                </form>
            ...
            <Modal
                ...
                footer={<p className="text-xs text-muted-foreground">Admin role cannot be renamed or have its permissions reduced.</p>}
            >
                {editingRole ? (
                    <form action={updateRoleAction} className="grid gap-3" onSubmit={() => setEditingRole(null)}>
                        ...
                        {editingRole.slug === "admin" ? editingRole.permissions.map((permission) => <input key={permission} type="hidden" name="permissions" value={permission} />) : null}
                        <label className="grid gap-1 text-xs uppercase text-muted-foreground">
                            Name
                            <input
                                name="name"
                        ...
                        <div className="flex justify-end gap-2">
                            <button type="button" onClick={() => setEditingRole(null)} className="rounded-md border border-border px-4 py-2 text-sm text-foreground transition hover:bg-muted">
                                Cancel
                            </button>
                            <button type="submit" className="rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground transition hover:bg-primary/90">
                                Save Changes
            ...
            <Modal
                ...
                footer={<p className="text-xs text-muted-foreground">Roles in use must be reassigned before deletion. Admin role is not deletable.</p>}
            >
                {deletingRole ? (
                    <form action={deleteRoleAction} className="grid gap-3" onSubmit={() => setDeletingRole(null)}>
                        ...
                        <div className="flex justify-end gap-2">
                            <button type="button" onClick={() => setDeletingRole(null)} className="rounded-md border border-border px-4 py-2 text-sm text-foreground transition hover:bg-muted">
                                Cancel
                            </button>
                            <button type="submit" className="rounded-md bg-destructive px-4 py-2 text-sm font-medium text-destructive-foreground transition hover:bg-destructive/80">
                                Delete Role
                            </button>
                        </div>
                    </form>
