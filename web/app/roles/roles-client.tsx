              ...
              <tr key={role.id} className="transition hover:bg-muted/70">
                <td className="px-4 py-3 font-medium text-foreground">
                  <div className="flex items-center gap-2">
                    <span>{role.name}</span>
                    {role.slug === 'admin' ? (
                      <span className="rounded-full border-0 bg-destructive px-2 py-0.5 text-xs font-medium uppercase tracking-wide text-destructive-foreground">
                        Admin
                      </span>
                    ) : role.isSystem ? (
      ...
      <Modal
        ...
        footer={
          <p className="text-xs text-muted-foreground">
            Permissions accept dotted keys, e.g. users.manage
          </p>
        }
      >
        <form
