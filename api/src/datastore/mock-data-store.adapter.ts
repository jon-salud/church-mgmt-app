...
export class MockDataStoreAdapter implements DataStore {
    ...
    async updateGroupMember(groupId: string, userId: string, input: Parameters<MockDatabaseService["updateGroupMember"]>[2]) {
        return this.mock.updateGroupMember(groupId, userId, input);
    }

    async removeGroupMember(groupId: string, userId: string, input: Parameters<MockDatabaseService["removeGroupMember"]>[2]) {
    ...
    async createRequestType(name: string, hasConfidentialField: boolean, actorUserId: string, description?: string) {
