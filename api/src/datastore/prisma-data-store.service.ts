...
export class PrismaDataStore implements DataStore {
    ...
    async updateSettings(churchId: string, update: any) {
        const optionalFields = JSON.stringify(update);
    ...
    async getDashboardSnapshot(churchId: string) {
        ...
        const totalGivingLast30 = (contributionsLast30 as any[]).reduce((sum: number, entry: any) => sum + toNumber(entry.amount || 0), 0);
    ...
    async addGroupMember(groupId: string, input: Parameters<DataStore["addGroupMember"]>[1]): Promise<StoreReturn<"addGroupMember">> {
        throw new Error("addGroupMember is not yet implemented for Prisma data store");
    }

    async updateGroupMember(groupId: string, userId: string, input: Parameters<DataStore["updateGroupMember"]>[2]): Promise<StoreReturn<"updateGroupMember">> {
        throw new Error("updateGroupMember is not yet implemented for Prisma data store");
    }

    async removeGroupMember(groupId: string, userId: string, input: Parameters<DataStore["removeGroupMember"]>[2]): Promise<StoreReturn<"removeGroupMember">> {
    ...
    async createAnnouncement(_input: Parameters<DataStore["createAnnouncement"]>[0]): Promise<any> {
        throw new Error("Not implemented: createAnnouncement");
    }

    async updateAnnouncement(_id: string, _input: Parameters<DataStore["updateAnnouncement"]>[1]): Promise<any> {
    ...
    async updateContribution(_id: string, _input: Parameters<DataStore["updateContribution"]>[1]): Promise<any> {
    ...
    async exportContributionsCsv(_input?: Parameters<DataStore["exportContributionsCsv"]>[0]): Promise<any> {
    ...
    async createSession(email: string, provider: string, requestedRole?: string) {
        ...
        const churchId = await this.getPrimaryChurchId();
        const existingRole = (user.churches as any[]).find((churchUser: any) => churchUser.churchId === churchId && (!requestedRole || churchUser.role === requestedRole));
        if (!existingRole) {
    ...
    async upsertUserFromOAuth(input: { provider: "google" | "facebook"; providerUserId: string; email: string; firstName?: string; lastName?: string; picture?: string }) {
