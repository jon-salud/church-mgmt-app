...
export class PrismaDataStore implements DataStore {
  ...
  async getDashboardSnapshot(churchId: string) {
    const [memberCount, groupCount, upcomingEvents, announcements, reads, contributionsLast30] =
      await Promise.all([
    ...
    const readsLookup = new Map(
      (reads as any[]).map((read: any) => [read.announcementId, read._count._all] as const)
    );
  ...
  async createSession(email: string, provider: string, requestedRole?: string) {
    let user = await this.client.user.findUnique({
      where: { primaryEmail: email },
      include: { churches: true, profile: true },
    });
    if (!user) {
    ...
    const existingRole = (user.churches as any[]).find(
      (churchUser: any) =>
        churchUser.churchId === churchId && (!requestedRole || churchUser.role === requestedRole)
    );
    if (!existingRole) {
  ...
  async getSessionByToken(token?: string) {
    ...
    return {
      ...
      user: {
        ...session.user,
        roles: (session.user.churches as any[]).map((churchUser: any) => ({
          churchId: churchUser.churchId,
          role: churchUser.role,
        })),
      },
  ...
  async upsertUserFromOAuth(input: {
    ...
    if (!user) {
      ...
      const hasChurch = (user.churches as any[]).some(
        (churchUser: any) => churchUser.churchId === churchId
      );
      if (!hasChurch) {
        await this.client.churchUser.upsert({
          where: { churchId_userId: { churchId, userId: user.id } },
  ...
  async updatePastoralCareTicket(
    id: string,
    input: Parameters<DataStore['updatePastoralCareTicket']>[1]
  ) {
