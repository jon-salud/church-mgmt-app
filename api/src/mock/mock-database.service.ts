...
export class MockDatabaseService {
    ...
    getHouseholdById(id: string) {
        ...
        const members = this.users.filter((u) => u.profile.householdId === id).map((u) => this.buildUserPayload(u));
        return {
            ...clone(household),
            members,
        };
    ...
    listUsers(query?: string) {
        ...
        const list = this.users
            .filter((user) => {
                ...
                return user.primaryEmail.toLowerCase().includes(lower) || profile.firstName.toLowerCase().includes(lower) || profile.lastName.toLowerCase().includes(lower);
            })
            .map((user) => {
    ...
    createUser(input: UserCreateInput) {
        const emailTaken = this.users.some((user) => user.primaryEmail.toLowerCase() === input.primaryEmail.toLowerCase());
        if (emailTaken) {
    ...
    updateUser(id: string, input: UserUpdateInput) {
        ...
        if (
            ...
            this.users.some((record) => record.id !== id && record.primaryEmail.toLowerCase() === input.primaryEmail!.toLowerCase())
        ) {
            throw new Error("A user with that email already exists");
        }
        ...
        if (input.roleIds) {
            ...
            const normalised = unique.length > 0 ? unique.map((identifier) => ({ churchId, roleId: this.resolveRoleId(identifier) })) : [{ churchId, roleId: this.getDefaultRoleId("member") }];
    ...
    recordAttendance(input: AttendanceInput) {
        ...
        const attendeeName = attendee ? `${attendee.profile.firstName} ${attendee.profile.lastName}`.trim() : input.userId;
        ...
        if (existing) {
            ...
            this.createAuditLog({
                ...
                diff: previousStatus !== input.status ? { previousStatus, newStatus: input.status } : undefined,
                metadata: {
                    userId: input.userId,
    ...
    markAnnouncementRead(announcementId: string, userId: string) {
        const already = this.announcementReads.find((read) => read.announcementId === announcementId && read.userId === userId);
        if (already) {
    ...
    updateAnnouncement(id: string, input: AnnouncementUpdateInput) {
        ...
        const apply = <K extends keyof MockAnnouncement>(key: K, next: MockAnnouncement[K]) => {
            ...
            const hasChanged = isArray ? JSON.stringify(previous) !== JSON.stringify(next) : previous !== next;
            if (hasChanged) {
    ...
    recordContribution(input: ContributionInput) {
        ...
        const actor = this.users.find((user) => user.id === actorUserId);
        const memberName = member ? `${member.profile.firstName} ${member.profile.lastName}`.trim() : contribution.memberId;
        const actorName = actor ? `${actor.profile.firstName} ${actor.profile.lastName}`.trim() : actorUserId;
    ...
    exportContributionsCsv(filter?: { memberId?: string; fundId?: string; from?: string; to?: string }) {
        ...
        const lines = [
            ...
            ...contributions.map((entry) => {
                ...
                const memberName = user ? `${user.profile.firstName ?? ""} ${user.profile.lastName ?? ""}`.trim() || user.primaryEmail : entry.memberId;
    ...
    createAuditLog(input: AuditLogCreateInput) {
        ...
        const record: MockAuditLog = {
            id: input.entityId ? `audit-${input.entity}-${input.entityId}-${Date.now()}` : `audit-${Date.now()}-${Math.random().toString(16).slice(2, 6)}`,
            churchId,
            actorUserId: input.actorUserId,
            action: input.action,
            entity: input.entity,
    ...
    createRole(input: RoleCreateInput) {
        ...
        const permissions = input.permissions ? Array.from(new Set(input.permissions.map((entry) => entry.trim()).filter(Boolean))).sort() : [];
        const now = new Date().toISOString();
        const role: MockRole = {
            id: `role-${randomUUID()}`,
            churchId,
    ...
    updateRole(id: string, input: RoleUpdateInput) {
        ...
        if (input.name) {
            ...
            if (this.roles.some((other) => other.id !== id && other.churchId === role.churchId && other.name.toLowerCase() === name.toLowerCase())) {
    ...
    upsertUserFromOAuth(input: { provider: "google" | "facebook"; providerUserId: string; email: string; firstName?: string; lastName?: string; picture?: string }) {
        const churchId = this.getChurch().id;
    ...
    createChild(data: { householdId: string; fullName: string; dateOfBirth: string; allergies?: string; medicalNotes?: string; actorUserId: string }) {
    ...
    createPushSubscription(data: { endpoint: string; keys: { p256dh: string; auth: string }; userId: string }) {
        const subscription = {
            id: `sub-${randomUUID()}`,
            ...data,
            createdAt: new Date().toISOString(),
